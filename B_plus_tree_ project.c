...
int main(int argc, char* argv[])
{
	...
	while (1) {
		printf("1. se 탐색키  2. sp  3. rs 시작키 최종키  4. ex \n");
		printf("명령>");
		r_gets = gets(line);
		if (!r_gets) {
			printf("명령 입력 오류\n");
		...
		if (strcmp(command, "ex") == 0)
			break;
			 ...
			 sequential_print();  // 이 함수는 각자 개발할 것!!
...
void sequential_print() {
	int i, j;
	FILE* fp;
	type_ptr_datanode start, tptr;
	start = HEAD; //데이터의 처음을 start로 지정
	
	fp = fopen("sequential_print.txt", "w");
	
	for (j = 0; j < start->fill_cnt; j++) { //start에 있는 데이타 노드를 모두 출력
		fprintf(fp, "%s %d\n", start->rec[j].name, start->rec[j].leng);
		fflush(fp);
	}

	tptr = start->link; //다음 링크로 넘어감
	while (tptr != NULL) { //tptr이 null이 아닐 때까지 
		for (i = 0; i < tptr->fill_cnt; i++) { //각 노드별로 fillcount전까지 뽑아준다.
			fprintf(fp, "%s %d\n", tptr->rec[i].name, tptr->rec[i].leng);
			fflush(fp);
		}
		tptr = tptr->link; //계속 다음 링크로 넘겨주며 반복한다.
	}
	fclose(fp);
}//sequential_print
...
int range_search(char* key1, char* key2)
{
	...
	for (j = i; j < ptr1->fill_cnt; j++) { // i 이후의 모든 레코드들을 출력한다.
		fprintf(fp, "%s %d\n", ptr1->rec[j].name, ptr1->rec[j].leng); ///여기 에러 발생
		fflush(fp);
		count++;
	}
