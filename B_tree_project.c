...
//       (주의: 회사명은 중간에 space 글자를 포함할 수도 있음.)
...
int insert_arec(ele in_rec) {	//하나의 레코드를 삽입  key = 회사명  
	...
	do {
		for (i = 0; i < curr->fill_cnt; i++) {
			...
			else if (strcmp(in_rec.name, curr->rec[i].name) == 0) { // 같은 이름이 있을 경우 종료
				printf("동일키 이미 존재로 안 넣은: %s\n", in_rec.name);
				return 0;
			}
		} 
		child = curr->ptr[i]; 
	...
	do { // curr는 리프노드이다.
		...
		if (curr->fill_cnt < MAXK) {
			for (i = 0; i < curr->fill_cnt; i++) 
				if (strcmp(in_rec.name, curr->rec[i].name) < 0) break; // 넣어야 할 rec번호 찾기	
					
			for (j = curr->fill_cnt; j > i; j--) { // 한 칸씩 옆으로 shift하고 넣기
					curr->ptr[j + 1] = curr->ptr[j];
					curr->rec[j] = curr->rec[j - 1];
			}
			curr->rec[i] = in_rec;    //위에서 찾은 rec 번호에 in_rec넣기
			curr->ptr[i + 1] = child; 
			curr->fill_cnt++;
			...
			// curr node is full
			for (i = 0; i < MAXK; i++) {
				if (strcmp(in_rec.name, curr->rec[i].name) < 0)  // curr에서 in_rec이 들어가야하는 위치를 찾는다.
					break;
			}

			bnode.ptr[0] = curr->ptr[0];  // bignode에 위에서 찾은 위치 전까지 담는다.
			for (j = 0; j < i; j++) {
				bnode.rec[j] = curr->rec[j];
				bnode.ptr[j + 1] = curr->ptr[j + 1];
			}

			bnode.rec[j] = in_rec;   // bignode에 in_rec을 담아준다.
			bnode.ptr[j + 1] = child;
			j++;

			while (i < MAXK) {
				bnode.rec[j] = curr->rec[i];  // bignode에 curr에 남은 나머지를 넣어준다.
			...
			curr->fill_cnt = HALFK;  
			curr->ptr[i] = bnode.ptr[i];   //bnode.ptr[i]는 curr.ptr[i]를 가리킨다.
			curr->ptr[i + 1] = NULL;

			new_ptr = (nodeptr)malloc(sizeof(node)); //뒤의 남은 반을 위해 malloc으로 자리할당
			...
			child = new_ptr;  //new_ptr을 child에 넣음
			...
			if (top >= 0) {		// 스택이 emtpy 가 아닐 경우 
				...
				tptr = (nodeptr)malloc(sizeof(node)); //새 root 노드
				tptr->rec[0] = in_rec;
				tptr->ptr[0] = curr;
				tptr->ptr[1] = child;
				tptr->fill_cnt = 1;
				root = tptr; //curr의 부모노드로 만듬
				total_height++;
...
void redistribution(nodeptr father, nodeptr l_sibling, nodeptr r_sibling, char wcase, int j) {
	...
	if (wcase == 'L')
		j = j - 1;  //l_sibling과 cur(=s_bling) 사이의 레코드의 index.
	else if (wcase == 'R')
		j = j; // curr(=l_sibling)과 r_siling 사이의 레코드의 index.
...
int B_tree_deletion(char* out_key) {
	...
	do {
		...
		if (found == 1)
			...
			Pt = curr->ptr[i];
			if (Pt) {   //curr를 자식으로 갱신하므로써 내려감
				push(curr);
				curr = Pt;
			}
			else break; // 내려갈 자식이 없다. 즉, curr는 리프노드이다.
		}
	...
	if (curr->ptr[0]) {   // curr node is not a leaf node  
	   // We need to find successor of out_key ;
		Pt = curr->ptr[i+1]; // 일단 먼저 d_rec의 바로 우측의 포인터로 내려간다.
		push(curr);
		// 가장 왼쪽 포인터를 따라내려 간다.
		while (Pt->ptr[0]) {
			push(Pt);  //중간에 거치는 노드들을 스택이 넣는다.
			Pt = Pt->ptr[0];
		}
		curr->rec[i] = Pt->rec[0];  // successor는 rec0에 있음. 이것으로 d_rec을 덮어쓴다.
		curr = Pt;// curr가 successor를 가진 리프노드를 가리키게 한다.
		strcpy(out_key, Pt->rec[0].name); // successor를 지우는 문제가 된다.
	...
	do {
		...
		if (r_OK || l_OK) { // 재분배가 가능함.
		   ...
		   //  Let rightptr point to the right one of curr and sibling chosen to merge ;
			...
			// Now, i has index of record of father between leftptr and rightptr.
			j = leftptr->fill_cnt;
